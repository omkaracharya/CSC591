...
def predict_rating(model, nodedict, user, movie):
    ...
    """
    # YOUR CODE HERE
    highest_similarity_i = 0
    highest_similarity = 0
    for i in xrange(0, 5):
        ratingname = movie + "_" + str(i + 1)
        similarity = model.similarity(nodedict[ratingname].id, nodedict[user].id)
        if similarity > highest_similarity:
            highest_similarity = similarity
            highest_similarity_i = i + 1
    return highest_similarity_i
