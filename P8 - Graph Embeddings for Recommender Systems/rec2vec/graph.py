...
def records_to_graph():
    ...
    idx = 0
    for movieKey, movieValue in movies.items():
        tempnode = Node(id=idx, name=movieKey, type='movie')
        nodelist.append(tempnode)
        nodedict[movieKey] = tempnode
        idx += 1

    for director in directors:
        tempnode = Node(id=idx, name=director, type='director')
        nodelist.append(tempnode)
        nodedict[director] = tempnode
        idx += 1

    for actor in actors:
        tempnode = Node(id=idx, name=actor, type='actor')
        nodelist.append(tempnode)
        nodedict[actor] = tempnode
        idx += 1

    for genre in genres:
        tempnode = Node(id=idx, name=genre, type='genre')
        nodelist.append(tempnode)
        nodedict[genre] = tempnode
        idx += 1

    for user in ratings.keys():
        tempnode = Node(id=idx, name=user, type='user')
        nodelist.append(tempnode)
        nodedict[user] = tempnode
        idx += 1

    for movieKey, movieValue in movies.items():
        for i in xrange(0, 5):
            ratingname = movieKey + "_" + str(i + 1)
            tempnode = Node(id=idx, name=ratingname, type='rating')
            nodelist.append(tempnode)
            nodedict[ratingname] = tempnode
            idx += 1
    ...
    for user in ratings.keys():
        for movie, rating in ratings[user].items():
            ratingname = movie + "_" + str(rating)
            nodedict[user].neighbors.append(nodedict[ratingname])
            nodedict[ratingname].neighbors.append(nodedict[user])

    # Add edges between movies and directors

    for movieKey, movieValue in movies.items():
        if movieValue.director:
            nodedict[movieKey].neighbors.append(nodedict[movieValue.director])
            nodedict[movieValue.director].neighbors.append(nodedict[movieKey])

    # Add edges between movies and actors

    for movieKey, movieValue in movies.items():
        for actor in movies[movieKey].actors:
            nodedict[movieKey].neighbors.append(nodedict[actor])
            nodedict[actor].neighbors.append(nodedict[movieKey])

    # Add edges between movies and genres

    for movieKey, movieValue in movies.items():
        for genre in movies[movieKey].genres:
            nodedict[movieKey].neighbors.append(nodedict[genre])
            nodedict[genre].neighbors.append(nodedict[movieKey])
    ...
    for movieKey, movieValue in movies.items():
        for i in xrange(0, 5):
            ratingname = movieKey + "_" + str(i + 1)
            nodedict[movieKey].neighbors.append(nodedict[ratingname])
            nodedict[ratingname].neighbors.append(nodedict[movieKey])
